#!/usr/bin/env python3
"""
S2：成交 → (date, expiry, strike) 日度面板（ETH_risk_premia_plan.md §1.1 Step 2）。

从 S1 合并长表（或列兼容的 CSV/Parquet）解析 Deribit ETH 期权 ``instrument_name``，
在 **清洗后、按日聚合前** 可选输出 **逐笔** 描述性统计（allign with BTC Table 1 format: Call/Put by columns
...
IV：Deribit 成交里 ``iv`` 多为 **0–100 in percentage**；本脚本默认输出 **小数年化波动**
（除以 100），与 S3 **默认**（不加 ``--iv-already-percent``）一致。

Upstream：``S1_consolidate_eth_options_parquet.py`` → ``eth_options_fullsample.csv``
Downstream：``S3_prepare_moneyness_eth.py``（列：date, expiry, K, spot, IV, quantity）
"""
