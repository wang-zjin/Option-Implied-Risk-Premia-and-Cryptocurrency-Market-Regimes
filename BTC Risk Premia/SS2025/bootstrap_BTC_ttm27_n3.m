function [theta_0,theta_b0_0,LL_0,theta_boot,theta_b0_boot,LL_boot,fig1,tab4,tab5,tab6] = bootstrap_BTC_n3(reps,T,block,N,ln_fQ_t,ln_sig_t,ln_fQ,r30,r_vec,del,lnRf,do_estimate,do_fig1,do_tab4,do_tab5,do_tab6)
...
for b=1:reps
    ...
    if do_fig1
       if N(3)~=3 
          disp('Figure 1 objects can only be computed for N=3'); return;
       else
       theta_b        = reshape(theta_boot(b,3,1:4),[4,1]);
       lnRf_b         = lnRf(obs_b);
       [~,~,~,~,fP_b] = LL(theta_b,N(3),ln_fQ_b,r30_b,ln_sig_t_b,r_vec,ln_fQ_t_b,del); % normalized density on every day
...
if do_estimate && all(N(:)'==[1 2 3]) % We use N=1;2 while the original SS2025JFE paper set N=1:5
   if isgpuarray(LL_boot)
      theta_boot = gather(theta_boot);
      theta_b0_boot = gather(theta_b0_boot);
      LL_boot = gather(LL_boot);       
   end
   save data_outputs/bootstrap_estimates_BTC_n3 theta_boot theta_b0_boot LL_boot theta_0 theta_b0_0 LL_0
end
