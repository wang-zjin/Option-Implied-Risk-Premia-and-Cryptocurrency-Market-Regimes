...
%
% BP_VRP_SS=nan(6,3);
% BP_VRP_SS(2,:)=[mean(ER_P),mean(ER_P(index0,:)),mean(ER_P(index1,:))]*365/45;
% BP_VRP_SS(3,:)=[mean(IR),mean(IR(index0,:)),mean(ER_Q(index1,:))]*365/45;
% %BP_VRP_SS(3,:)=[mean(ER_Q),mean(ER_Q(index0,:)),mean(ER_Q(index1,:))]*365/45;
% BP_VRP_SS(3,:)=[mean(IR),mean(IR(index0,:)),mean(IR(index1,:))];
% BP_VRP_SS(1,:)=BP_VRP_SS(2,:)-BP_VRP_SS(3,:);
% BP_VRP_SS(5,:)=[mean(var_Q),mean(var_Q(index0,:)),mean(var_Q(index1,:))]*365/45;
% BP_VRP_SS(6,:)=[mean(var_P),mean(var_P(index0,:)),mean(var_P(index1,:))]*365/45;
...
E_P_OA=mean(fP * R_vec) * 365/45 * del;
E_P_HV=mean(fP(index0,:) * R_vec) * 365/45 * del;
E_P_LV=mean(fP(index1,:) * R_vec) * 365/45 * del;
...
% sig_t is annualised monthly realized volatility

tb_RV_cluster0 = [tb_date_cluster0, table(sig_t(index0,1).^2 * 365/ttm,'VariableNames',"RV"),table(zeros(size(tb_date_cluster0)),'VariableNames',"Cluster")];
tb_RV_cluster1 = [tb_date_cluster1, table(sig_t(index1,1).^2 * 365/ttm,'VariableNames',"RV"),table(ones(size(tb_date_cluster1)),'VariableNames',"Cluster")];
tb_RV_overall = [tb_date_overall, table(sig_t(:,1).^2 * 365/ttm,'VariableNames',"RV"),table(common_dates.Cluster,'VariableNames',"Cluster")];
%% Introduce VIX
VIX = readtable(['data_prep/data_BTC/btc_vix_EWA_',num2str(ttm),'.csv']);
VIX.Date = datetime(VIX.Date, "Format", "uuuuMMdd", "InputFormat", "uuuuMMdd");
...
%% Save tb_VIX_RV_VRP_HV, tb_VIX_RV_VRP_LV (ttm=45, separate files to avoid overwriting ttm=45)
writetable(tb_VIX_RV_VRP_cluster0,"data_prep/data_BTC/RiskPremia/multivariate_clustering_9_27_45/Variance_Risk_Premium/VRP_VIX_Corsi_RV_HV_ttm45.csv");
writetable(tb_VIX_RV_VRP_cluster1,"data_prep/data_BTC/RiskPremia/multivariate_clustering_9_27_45/Variance_Risk_Premium/VRP_VIX_Corsi_RV_LV_ttm45.csv");

%% Report VRP (ttm=45)
tb_VIX_RV_VRP_cluster0 = readtable("data_prep/data_BTC/RiskPremia/multivariate_clustering_9_27_45/Variance_Risk_Premium/VRP_VIX_Corsi_RV_HV_ttm45.csv");
tb_VIX_RV_VRP_cluster1 = readtable("data_prep/data_BTC/RiskPremia/multivariate_clustering_9_27_45/Variance_Risk_Premium/VRP_VIX_Corsi_RV_LV_ttm45.csv");
...
% Q: Q density vs VIX
[~,tbl_VRP_HV,stats_VRP_HV] = anova1([tb_VIX_RV_VRP_cluster0.VRP;tb_VIX_RV_VRP_overall.VRP],cluster_label_0_VIX);
[~,tbl_VRP_LV,stats_VRP_LV] = anova1([tb_VIX_RV_VRP_cluster1.VRP;tb_VIX_RV_VRP_overall.VRP],cluster_label_1_VIX);
[~,tbl_VarQ_HV,stats_VarQ_HV] = anova1([tb_VIX_RV_VRP_cluster0.Q_variance_VIX;tb_VIX_RV_VRP_overall.Q_variance_VIX],cluster_label_0_VIX);
[~,tbl_VarQ_LV,stats_VarQ_LV] = anova1([tb_VIX_RV_VRP_cluster1.Q_variance_VIX;tb_VIX_RV_VRP_overall.Q_variance_VIX],cluster_label_1_VIX);
[~,tbl_VarP_HV,stats_VarP_HV] = anova1([tb_VIX_RV_VRP_cluster0.RV;tb_VIX_RV_VRP_overall.RV],cluster_label_0_VIX);
[~,tbl_VarP_LV,stats_VarP_LV] = anova1([tb_VIX_RV_VRP_cluster1.RV;tb_VIX_RV_VRP_overall.RV],cluster_label_1_VIX);
...
VRP_difference(7,:)=[height(tb_VIX_RV_VRP_overall),height(tb_VIX_RV_VRP_cluster0),height(tb_VIX_RV_VRP_cluster1)];
clear info;
info.rnames = strvcat('.','VRP','p-value','Q ann variance','p-value','P ann variance','p-value','Num of obser');
info.cnames = strvcat('Overall','Cluster 0','Cluster 1');
info.fmt    = '%10.2f';
disp('Q average vs. VIX average (ttm=45)')
