...
%
% BP_VRP_SS=nan(6,3);
% BP_VRP_SS(2,:)=[mean(ER_P),mean(ER_P(index0,:)),mean(ER_P(index1,:))]*365/9;
% BP_VRP_SS(3,:)=[mean(IR),mean(IR(index0,:)),mean(ER_Q(index1,:))]*365/9;
% %BP_VRP_SS(3,:)=[mean(ER_Q),mean(ER_Q(index0,:)),mean(ER_Q(index1,:))]*365/9;
% BP_VRP_SS(3,:)=[mean(IR),mean(IR(index0,:)),mean(IR(index1,:))];
% BP_VRP_SS(1,:)=BP_VRP_SS(2,:)-BP_VRP_SS(3,:);
% BP_VRP_SS(5,:)=[mean(var_Q),mean(var_Q(index0,:)),mean(var_Q(index1,:))]*365/9;
% BP_VRP_SS(6,:)=[mean(var_P),mean(var_P(index0,:)),mean(var_P(index1,:))]*365/9;
...
E_P_OA=mean(fP * R_vec) * 365/9 * del;
E_P_HV=mean(fP(index0,:) * R_vec) * 365/9 * del;
E_P_LV=mean(fP(index1,:) * R_vec) * 365/9 * del;
...
%% Save tb_VIX_RV_VRP_HV, tb_VIX_RV_VRP_LV (ttm=9, separate files to avoid overwriting ttm=9)
writetable(tb_VIX_RV_VRP_cluster0,"data_prep/data_BTC/RiskPremia/multivariate_clustering_9_27_45/Variance_Risk_Premium/VRP_VIX_Corsi_RV_HV_ttm9.csv");
writetable(tb_VIX_RV_VRP_cluster1,"data_prep/data_BTC/RiskPremia/multivariate_clustering_9_27_45/Variance_Risk_Premium/VRP_VIX_Corsi_RV_LV_ttm9.csv");

%% Report VRP (ttm=9)
tb_VIX_RV_VRP_cluster0 = readtable("data_prep/data_BTC/RiskPremia/multivariate_clustering_9_27_45/Variance_Risk_Premium/VRP_VIX_Corsi_RV_HV_ttm9.csv");
tb_VIX_RV_VRP_cluster1 = readtable("data_prep/data_BTC/RiskPremia/multivariate_clustering_9_27_45/Variance_Risk_Premium/VRP_VIX_Corsi_RV_LV_ttm9.csv");
...
info.cnames = strvcat('Overall','Cluster 0','Cluster 1');
info.fmt    = '%10.2f';
disp('Q average vs. VIX average (ttm=9)')
